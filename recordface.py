...
class FaceCaptureAndAugmentation:
    def __init__(self, user_id, output_dir="data/dataset_faces", num_faces_to_save=500, augmentation_limits=None):
        ...
        self.output_dir_no_aug = os.path.join(output_dir, user_id, "no-aug")
        self.output_dir_aug = os.path.join(output_dir, user_id, "aug")
        ...
        os.makedirs(self.output_dir_no_aug, exist_ok=True)
        os.makedirs(self.output_dir_aug, exist_ok=True)
    ...
    def save_faces(self):
        # Save non-augmented faces in the "no-aug" folder
        for idx, face in enumerate(self.captured_faces):
            face_filename = os.path.join(self.output_dir_no_aug, f"face_{idx}.jpg")
            cv2.imwrite(face_filename, face)

        # Once original faces are saved, augment and save them in the "aug" folder
        self.augment_faces()
    ...
    def augment_faces(self):
        ...
        for aug_type in self.augmentation_types:
            ...
            for idx, face in enumerate(self.captured_faces):
                ...
                face_filename = os.path.join(self.output_dir_aug, f"{aug_type}_face_{idx}_{count}.jpg")
                cv2.imwrite(face_filename, augmented_face)
                count += 1
                augmented_count += 1

        print(f"Saved {self.face_count} non-augmented faces and {augmented_count} augmented faces.")
...
# Example usage:
face_capture = FaceCaptureAndAugmentation(user_id="6420063")
